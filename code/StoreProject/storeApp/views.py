...
def loadProducts(request):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			active = request.POST.get('productActive')
			activeValue = False
			if active is 1:
				activeValue = True
			activeFinal = activeValue
			productQuantity = int(request.POST.get('productQuantity'))
			#supplies = str(request.POST.get('productSupplier'))
			#
			newProduct = Product(product_id=productId, product_name=productName, product_active=activeFinal, product_stock_quantity=productQuantity,
	...
	if request.method == 'POST':
		...
		if 'registerUser' in request.POST:
			...
			if form.is_valid():
				...
				enteredFName = request.POST.get('fName')
				enteredLName = request.POST.get('lName')
				...
				enteredCheck = request.POST.get('passwordCheck')
				enteredAddress = request.POST.get('address')
